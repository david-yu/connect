...
// MustExecContext executes a SQL statement with context and fails the test on error.
...
//
// The test is skipped on darwin (DB2 CLI library is Linux-only) and when the
// shared container is unavailable (Docker not running, no icr.io credentials, etc.).
func SetupTest(t *testing.T) *TestDB {
	t.Helper()

	if runtime.GOOS == "darwin" {
		t.Skip("skipping DB2 integration test: DB2 CLI library is Linux-only (no macOS build from IBM)")
	}
...
func (db *TestDB) enableASNCDCLocal(schema string, tables []string) {
	...
	// Ensure asncap is still running (it may have exited since startup).
	if err := startAsncapLocal(); err != nil {
	...
	if out, _ := exec.Command("pgrep", "-f", "capture_server="+db2Database).Output(); strings.TrimSpace(string(out)) == "" {
	...
	if logOut, _ := os.ReadFile("/tmp/asncap.log"); len(logOut) > 0 {
		tail := logOut
		if len(tail) > 3000 {
...
func createASNCDCTablesIfNeeded(ctx context.Context, db *sql.DB) error {
	...
	// Rewrite all ASN schema references to ASNCDC.
...
func addTableLocal(ctx context.Context, db *sql.DB, sourceSchema, sourceTable string) error {
	...
	// Idempotent: skip if the CD table already exists.
	var n int
	if err := db.QueryRowContext(ctx,
	...
	if n > 0 {
		fmt.Printf("db2test: CD table ASNCDC.%s already exists\n", cdTable)
		return nil
	}

	// Step 1: Enable full-row logging on the source table.
	alterSQL := fmt.Sprintf(`ALTER TABLE "%s"."%s" DATA CAPTURE CHANGES`, sourceSchema, sourceTable)
	...
	// Detect the correct IBMSNAP_COMMITSEQ column size from IBMSNAP_REGISTER.
	// DB2 ≤ 11.x uses CHAR(10) FOR BIT DATA; DB2 12.1+ uses CHAR(16) FOR BIT DATA.
	// asncap will fail to INSERT if the CD table uses a smaller type than the LSN size.
	commitSeqByteLen := cdNewSynchpointHexLen(ctx, db) / 2
	if commitSeqByteLen <= 0 {
		commitSeqByteLen = 10
	}
	...
	for colRows.Next() {
		var c colDef
		if err := colRows.Scan(&c.Name, &c.TypeName, &c.Len, &c.Scale); err != nil {
			return err
		}
		cols = append(cols, c)
	}
	...
	// Step 2: CREATE the CD table.
	//
	// IBM's ADDTABLE uses VARCHAR FOR BIT DATA (not CHAR) and puts COMMITSEQ first,
	// then INTENTSEQ, then OPERATION — this matches what asncap expects when writing
	// rows. Column order: COMMITSEQ, INTENTSEQ, OPERATION, <user cols> (no LOGMARKER).
	var sb strings.Builder
	fmt.Fprintf(&sb, `CREATE TABLE ASNCDC."%s" (`, cdTable)
	fmt.Fprintf(&sb, "\n  IBMSNAP_COMMITSEQ VARCHAR(%d) FOR BIT DATA NOT NULL,", commitSeqByteLen)
	fmt.Fprintf(&sb, "\n  IBMSNAP_INTENTSEQ VARCHAR(%d) FOR BIT DATA NOT NULL,", commitSeqByteLen)
	...
	// Step 3: Register in IBMSNAP_REGISTER.
	//
	...
	pruncntlSQL := fmt.Sprintf(`
		INSERT INTO ASNCDC.IBMSNAP_PRUNCNTL (
			...
			(SELECT CURRENT SERVER FROM SYSIBM.SYSDUMMY1),
			'ASNCDC', '%s',
			(SELECT CASE WHEN MAX(CAST(MAP_ID AS INT)) IS NULL
			             THEN CAST(1 AS VARCHAR(10))
			             ELSE CAST(CAST(MAX(MAP_ID) AS INT) + 1 AS VARCHAR(10))
			        END
...
// cdNewSynchpointHexLen returns 2× the byte length of CD_NEW_SYNCHPOINT in
// IBMSNAP_REGISTER (i.e. the number of hex characters expected by X'...'
// literals). Falls back to 20 (CHAR(10) FOR BIT DATA) if the schema cannot
// be queried.
func cdNewSynchpointHexLen(ctx context.Context, db *sql.DB) int {
	var length int
	if err := db.QueryRowContext(ctx,
		"SELECT LENGTH FROM SYSCAT.COLUMNS WHERE TABSCHEMA='ASNCDC' AND TABNAME='IBMSNAP_REGISTER' AND COLNAME='CD_NEW_SYNCHPOINT'",
	).Scan(&length); err == nil && length > 0 {
		return length * 2
	}
	return 20
}
