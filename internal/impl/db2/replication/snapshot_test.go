...
func TestConvertDB2Value(t *testing.T) {
	...
	assert.Equal(t, "Alice", convertDB2Value([]byte("Alice"), colTypes[0]))
	// []byte with BLOB → raw bytes
	assert.Equal(t, []byte{0xDE, 0xAD}, convertDB2Value([]byte{0xDE, 0xAD}, colTypes[1]))
	// int64 passes through unchanged
	assert.Equal(t, int64(42), convertDB2Value(int64(42), colTypes[2]))
	// nil returns nil regardless of column type
	assert.Nil(t, convertDB2Value(nil, colTypes[0]))
	// []byte with CHAR type → string
	db2 := openFakeDB(t, &replFakeHandlers{
		query: func(_ string, _ []driver.Value) ([]string, [][]driver.Value, error) {
			return []string{"C"}, [][]driver.Value{{[]byte("abc")}}, nil
		},
		colTypes: func(_ string) map[string]string {
			return map[string]string{"C": "CHAR"}
		},
	})
	tx2, err := db2.Begin()
	require.NoError(t, err)
	defer tx2.Rollback() //nolint:errcheck
	rows2, err := tx2.QueryContext(context.Background(), "SELECT C FROM T")
	require.NoError(t, err)
	defer func() { require.NoError(t, rows2.Close()); require.NoError(t, rows2.Err()) }()
	ct2, err := rows2.ColumnTypes()
	require.NoError(t, err)
	assert.Equal(t, "abc", convertDB2Value([]byte("abc"), ct2[0]))
}
...
func TestCaptureCurrentCSN(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "table does not exist returns zero CSN",
			queryFunc: func(_ string, _ []driver.Value) ([]string, [][]driver.Value, error) {
				return nil, nil, fmt.Errorf("table ASNCDC.IBMSNAP_REGISTER does not exist")
			},
			wantCSN: 0,
		},
		{
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			s := NewSnapshotter(db, SnapshotConfig{Schema: "MYSCHEMA"}, Version{})
...
func TestDiscoverPrimaryKeys(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			pks, err := s.discoverPrimaryKeys(context.Background(), tx, "EMPLOYEES")
			if tc.wantErr {
				require.Error(t, err)
				return
			}
...
func TestGetTableColumns(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			errMode := tc.wantErr && tc.rows == nil
			db := openFakeDB(t, &replFakeHandlers{
				query: func(_ string, _ []driver.Value) ([]string, [][]driver.Value, error) {
					_ = errMode // empty rows are returned via tc.rows being nil below
					if tc.wantErr && tc.name == "query error propagates" {
			...
			cols, err := s.getTableColumns(context.Background(), tx, "EMPLOYEES")
			if tc.wantErr {
				require.Error(t, err)
				return
			}
...
func TestFetchBatch(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := s.fetchBatch(context.Background(), tx, "EMPLOYEES", tc.columns, tc.pks, tc.lastKeyValues)
			require.NoError(t, err)
